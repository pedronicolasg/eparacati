document.addEventListener('DOMContentLoaded', function() {
  ...
  const isGridView = document.querySelector('.grid-cols-1.md\\:grid-cols-2.lg\\:grid-cols-3') !== null;
  const equipmentSelector = isGridView 
    ? '.grid-cols-1.md\\:grid-cols-2.lg\\:grid-cols-3 > div'
    : '.space-y-4 > div';
  
  const equipmentItems = document.querySelectorAll(equipmentSelector);
  ...
  function filterEquipments(searchTerm) {
    ...
    if (visibleCount === 0 && searchTerm !== '') {
      if (!noResultsMessage) {
        const container = document.querySelector(isGridView ? 
