    ...
    public function create($name, $grade, $pdtId = null, $leaderId = null, $viceLeaderId = null, $startNewTransaction = true)
    {
        try {
            ...
            foreach ($roles as $userId => $role) {
                if ($userId) {
                    $this->userManager->updateRole($userId, $role);
                    $this->userManager->updateClass($userId, $id);
                }
            }
        ...
        try {
            ...
            foreach ($rows as $index => $row) {
                try {
                    ...
                    $roleIds = [];
                    foreach ([2, 3, 4] as $i) {
                        $roleIds[$i] = !empty($row[$i]) && filter_var($row[$i], FILTER_VALIDATE_EMAIL) ? $this->userManager->getInfo($row[$i], 'email', ['id'])['id'] : $row[$i];
                    }
        ...
        try {
            ...
            foreach ($roles as $roleId => $roleData) {
                if ($currentClass[$roleId] != $roleData['new']) {
                    $this->userManager->updateRole($currentClass[$roleId], $roleData['default']);
                    if ($roleData['new']) {
                        $this->userManager->updateRole($roleData['new'], $roleData['role']);
                    }
                }
            }
    ...
    public function delete($id, $deleteStudents = false)
    {
        try {
            ...
            foreach ($roles as $roleId => $role) {
                if ($class[$roleId]) {
                    $this->userManager->updateRole($class[$roleId], $role);
                }
            }
